...
<div class="px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto py-16 text-white">
            ...
            {#if $authStore.isAuthenticated && $authStore.userId == userId}
                <Link to="/editprofile/{$authStore.userId}">
                    <img
                        src="/edit.svg"
                        alt=""
                        class="absolute bottom-0 right-0 p-1 bg-transparent rounded-full text-white shadow-lg h-7 cursor-pointer hover:bg-[#F5AE55] transition duration-150"
                    />
                </Link>
            {/if}
        </div>
                ...
                {#if $authStore.isAuthenticated && $authStore.userId == userId}
                    <Link to="/editprofile/{$authStore.userId}">
                        <img
                            src="/edit.svg"
                            alt=""
                            class="p-1 bg-transparent rounded-full text-white shadow-lg h-7 cursor-pointer hover:bg-[#F5AE55] transition duration-150"
                        />
                    </Link>
                {/if}
            </div>
    ...
    <div class="flex justify-between items-end mb-6">
        <h2 class="text-3xl font-bold text-white">Works</h2>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {#if $authStore.isAuthenticated && $authStore.userId == userId}
            <Link
                to="/createpost"
                class="bg-gray-800 rounded-lg overflow-hidden shadow-xl hover:shadow-2xl transition duration-300 cursor-pointer border border-gray-800 hover:bg-gray-700 hover:border-[#F5AE55] hover:shadow-lg"
            >
                <div class="relative flex items-center justify-center">
                    <img src="/plusIcon.svg" alt="Create post" />
                </div>
            </Link>
        {/if}
